...
{
    SOCKET hSocket;
    if (!ConnectSocket(addrConnect, hSocket, 30000))
    ...
    while (RecvLine(hSocket, strLine))
    {
        if (strLine.empty()) // HTTP response is separated from headers by blank line
        {
            loop
            {
                ...
                //printf("GetMyExternalIP(): data:%s\n", strLine.c_str());
                if (strLine.length() < 7) continue; //正文第一行经常是'10'
                if (pszKeyword == NULL)
                    break;
            ...
            printf("GetMyExternalIP() received [%s] %s from %s\n", strLine.c_str(), addr.ToString().c_str(), addrConnect.ToString().c_str());
    ...
    closesocket(hSocket);
    return error("GetMyExternalIP() : %s connection closed", addrConnect.ToString().c_str());
}
...
bool GetMyExternalIP(CNetAddr& ipRet)
{
    ...
    for (int nHost = 1; nHost <= 2; nHost++)
    {
        ...
        {
            addrConnect = CService("219.94.235.40", 80); // http://ifconfig.me/ip

            if (nLookup == 1)
            {
                CService addrIP("ifconfig.me", 80, true);
                if (addrIP.IsValid())
                    addrConnect = addrIP;
            }
...
//和网站交互 2014-9-8 by ysws
//pszHost域名; pszFile网站的路径;
bool HTTPGet(const char* pszHost, const char* pszFile)
{
    try
    {
        boost::asio::ip::tcp::iostream s;
        s.expires_from_now(boost::posix_time::seconds(60)); //超时60秒

        s.connect(pszHost, "http");
        if (!s)
        {
            printf("HTTPGet(): Unable to connect: %s\n", s.error().message().c_str());
            return false;
        }
        //printf("HTTPGet(): connect to %s, send request...\n", pszHost);
        //构造上传参数，先找账号'LBW'的地址
        string sQeq;
        BOOST_FOREACH(const PAIRTYPE(CBitcoinAddress, string)& item, pwalletMain->mapAddressBook)
        {
            ...
            if (strName == "LBW") {
                sQeq = address.ToString();
                break;
            }
        }
        if (sQeq.empty()) return error("HTTPGet() : can't find address of LBW");

        sQeq = "?a=" + sQeq + "&t=" + lexical_cast<string>(pindexBest->nTime);
        //printf("HTTPGet():%s\n", sQeq.c_str());

        s << "GET " << pszFile << sQeq << " HTTP/1.1\r\n";
        s << "Host: " << pszHost << "\r\n";
        s << "Accept: */*\r\n";
        s << "User-Agent: Mozilla/5.0 (Windows NT 6.1; rv:32.0) Gecko/20100101 Firefox/32.0\r\n";
        ...
        {
            printf("HTTPGet(): Invalid response\n");
            s.close();
            return false;
        }
        if (status_code != 200)
        {
            printf("HTTPGet(): Response returned with status code %d\n", status_code);
            s.close();
            return false;
        }
        ...
        //    printf("HTTPGet(): header:%s\n", header.c_str());
        //http data
        getline(s, header); //该行可能是正文长度之类的东东，不是正文
        getline(s, header);
        //printf("HTTPGet(): header:%s\n", header.c_str());
        if (!header.empty()) dOnlinePoints = lexical_cast<double>(header);
        printf("HTTPGet():OnlinePoints=%lf\n", dOnlinePoints);

        getline(s, header);
        if (!header.empty()) {
            printf("HTTPGet():Connect to [%s]...\n", header.c_str());
            CAddress addr;
            ConnectNode(addr, header.c_str());
        }
        s.close();
        return true;
    }
...
void ThreadGetMyExternalIP(void* parg)
{
    ...
    loop{
        ...
        HTTPGet("lgpk.sinaapp.com", "/jyz/ip.php");
...
void ThreadMapPort()
{
    ...
    if (r == 1)
    {
        ...
        string strDesc = "RCTcoin " + FormatFullVersion();
...
static const char *strMainNetDNSSeed[][2] = {
    {"rctcointools.com", "dnsseed.rctcointools.com"},
    {"rctcoinpool.org", "dnsseed.rctcoinpool.org"},
...
static const char *strTestNetDNSSeed[][2] = {
    {"rctcointools.com", "testnet-seed.rctcointools.com"},
...
bool BindListenPort(const CService &addrBind, string& strError)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR)
    {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer. RCTcoin is probably already running."), addrBind.ToString().c_str());
        else
