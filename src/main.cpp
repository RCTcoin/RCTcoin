...
#if defined(NDEBUG)
# error "RCTcoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xbed1a0b85cf9eb460a32f245bb2a84a8e2c2b09777a476408dc2d8535abc0653");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // RCTcoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "RCTcoin Signed Message:\n";
...
{
    // RCTcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 840000); // RCTcoin: 840k blocks in ~4 years

    return nSubsidy + nFees;*/
}

static const int64 nTargetTimespan = 3.5 * 24 * 60 * 60; // RCTcoin: 3.5 days
static const int64 nTargetSpacing = 2.5 * 60; // RCTcoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // RCTcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // RCTcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // RCTcoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "RCT(recreation) 1st IPO at 2014-6-8 8:00 ~ 2014-6-18 8:00";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1402185600;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 1207807;
        ...
        assert(block.hashMerkleRoot == uint256("0x785c4b851f7b49d12edf6bef04a33663f4c4f34323ddbff0505ba06c91e1e00f"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // RCTcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("RCTcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("RCTcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static RCTcoinMiner(CWallet *pwallet)
{
    printf("RCTcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("rctcoin-miner");
    ...
    try { loop {
        ...
        int nJanGe = nStart - nTimeBestReceived;
        if (nJanGe < 150) {  //2.5 * 60
            if (nJanGe >= 0) {
                printf("start sleep %u sec\n", 150 - nJanGe);
                MilliSleep((151 - nJanGe) * 1000);
            }
        }
        //
        // Create new block
        ...
        printf("Running RCTcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
        ...
        //pblock->print();
        //改挖矿起始nNonce为随机值
        unsigned int nNonceStart = GetRand(0xffee0000);
        printf("nNonceStart at %u\n", nNonceStart);
        pblock->nNonce = nNonceStart;
    ...
    {
        printf("RCTcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&RCTcoinMiner, pwallet));
}
